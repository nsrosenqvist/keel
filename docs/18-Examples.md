...
Runnable croft projects under
[`examples/`](https://github.com/nsrosenqvist/croft/tree/main/examples)
in the source tree. Clone the repo, `cd` into one, and `croft
list` will show what's wired up.

## [`minimal`](https://github.com/nsrosenqvist/croft/tree/main/examples/minimal)

The smallest useful `croft.toml`: a couple of host-only recipes,
no containers, no services. Use as a starting template when you're
adding croft to a project that doesn't need compose.
...
## [`laravel-app`](https://github.com/nsrosenqvist/croft/tree/main/examples/laravel-app)

A complete Laravel + Docker Compose project, modeled on the
hand-rolled `dev` scripts croft was built to replace. Covers most
of the surface in one place.

Showcases:

- Compose backend with `default_service = "app"`.
- Recipes that exec inside containers (`croft artisan migrate`,
  `croft shell`, `croft test --filter Login`).
...
## [`install-flow`](https://github.com/nsrosenqvist/croft/tree/main/examples/install-flow)

`.croft/install/` ordered setup with optional + interactive steps.
Walks through `croft install`, single-step mode (`croft install
...
- `# @interactive: yes` plus `croft lib ask` for first-run secret
  prompts.
- Resume-after-failure prompt and `--restart`.

## [`hooks`](https://github.com/nsrosenqvist/croft/tree/main/examples/hooks)
...
- `croft doctor` reporting the resolved container + image plan.

## [`agents-upstream`](https://github.com/nsrosenqvist/croft/tree/main/examples/agents-upstream)
...
- `croft-agents.toml` upstream manifest with `[[file]]`,
  `[[dir]]`, and `mode = "once"`.
