...
`.scaffl/worktrees/<slug>.toml`. Each git worktree has its own
...
| Command | Notes |
|---|---|
| `scaffl worktree status` | Current slug, offset, isolation flag, derived env values. |
| `scaffl worktree list` | Every git worktree's computed offset, with collision warnings. |
| `scaffl worktree assign <slug> <n> [--local]` | Pin a slug to an offset. |

`scaffl worktree assign` without `--local` writes to `scaffl.toml`
(team-wide; commit it to share). With `--local`, writes to
`.scaffl/local.toml` (per-developer, this checkout only).
...
Tools invoked outside scaffl (`docker compose up` directly,
...
1. **Auto-write on every scaffl invocation.** The resolved `[env]`
   plus the three worktree-derived built-ins
   (`SCAFFL_WORKTREE_SLUG`, `_OFFSET`, `COMPOSE_PROJECT_NAME`) land
...
checkout under the repo, hot-reload scaffl into a different worktree
