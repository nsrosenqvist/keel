...
scaffl owns a small git-hook installer plus a native runner that
understands `.pre-commit-config.yaml`. No dependency on the
`pre-commit` binary; scaffl runs supported hook languages itself,
errors loudly on the rest.

## Two hook sources

scaffl recognises hooks from two places, both run by the same shim:

1. **`[hooks.<stage>]` in `scaffl.toml`** — native scaffl hooks.
...
2. **`.pre-commit-config.yaml`** at the project root — standard
   pre-commit-format file, parsed by scaffl directly. Both
...
```sh
scaffl hooks install               # default: pre-commit
scaffl hooks install --stages pre-commit,pre-push,post-merge
```
...
hook out of the way, re-running `scaffl hooks install` writes the
shim cleanly.

`scaffl hooks uninstall [--stages ...]` removes only scaffl-managed
shims (identified by the marker comment).

## Implicit stages

When `[worktrees].dotenv = "..."` is set, `scaffl hooks install`
...
across branch switches even when the developer skips scaffl. See
...
(via the shared
[`scaffl-cache`](https://github.com/nsrosenqvist/scaffl/tree/main/crates/scaffl-cache)
...
`scaffl install --update-hooks` force-refreshes the cache (clears
the entry, re-clones at the same rev). Useful when an upstream
moves a tag.

## What languages run natively

scaffl runs hooks whose effective language is `system` or `script`:
...
- **`script`** — the `entry` resolves to a script file inside the
  cached repo (or the project for `repo: local`); scaffl exec's it
  directly.
...
`scaffl hooks run <stage>` is what the installed shim invokes; you
can run it directly to debug:

```sh
scaffl hooks run pre-commit             # uses staged files
scaffl hooks run pre-push origin main   # forwards stage args
```
...
For a given stage, scaffl runs:

1. Every `[hooks.<stage>]` entry from `scaffl.toml`, in order, via
   the recipe runner.
...
- [Install Flow](./Install-Flow.md) — `scaffl install` includes a
  synthetic step that installs hook shims and prefetches external
  repos.
- [`examples/hooks/`](https://github.com/nsrosenqvist/scaffl/tree/main/examples/hooks)
