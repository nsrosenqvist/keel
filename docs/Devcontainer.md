...
scaffl can route terminal sessions and host-targeted recipes into a
project's [devcontainer](https://containers.dev/) instead of running
them on the host. The feature is **opt-in**: scaffl ignores
...
When enabled, scaffl auto-detects `.devcontainer/devcontainer.json`
...
| `scaffl shell` | (no command) | enters devcontainer |
| `scaffl shell --service app` | (no command) | enters compose service `app` (devcontainer toggle irrelevant) |
...
scaffl parses a subset of the spec — enough for the most common
...
- `remoteEnv` (merged on top of scaffl recipe env at `docker exec`)
- `remoteUser`
...
- `dockerComposeFile` — **rejected** with an error. Use scaffl's
...
scaffl owns the container's lifecycle: there's no separate "up" step
to run.

1. **First use** (TUI `n` press, recipe run, `scaffl shell`):
   - For `build` mode, build the image and tag it
     `scaffl-devcontainer-<project>:<sha>`, where `<sha>` is a hash
     of the dockerfile + build args. Skipped if the tag already
     exists.
   - `docker run -d` the container with `--name
     scaffl-devcontainer-<project>[-<worktree>]`, the workspace
...
2. **Subsequent uses**: `docker exec -it <name> ...`. If the
   container is stopped, scaffl `docker start`s it first.

To rebuild after editing the Dockerfile or build args, change a
build input — the hash changes, scaffl detects the missing tag, and
rebuilds. To rebuild without changing inputs:

```sh
docker rm -f scaffl-devcontainer-<project>
docker image rm scaffl-devcontainer-<project>:<tag>
```
...
- Container name: `scaffl-devcontainer-<project-slug>-<worktree-slug>`
- Bind mount: the worktree's own root → `workspaceFolder`
- Labels: `scaffl.devcontainer.root=<absolute path>`,
  `scaffl.devcontainer.worktree=<slug>`
...
`scaffl doctor` reports the active devcontainer setup (config path,
