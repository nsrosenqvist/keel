...
`croft watch <recipe>` re-runs `<recipe>` whenever watched files
change.
...
```sh
croft watch test
croft watch test --filter Login
croft watch lint --path src --path tests --debounce-ms 500
```
...
Trailing args are forwarded to the recipe in the same way `croft
