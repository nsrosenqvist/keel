...
shared agent instructions and skills via `keel agents`.

## Layout

```
keel-agents.toml      # the manifest keel reads downstream
...
```sh
keel agents install      # pull pinned upstream into the project
keel agents update       # re-resolve revs (auto-refetches floating refs)
keel agents status       # per-source pinned rev + per-file drift
keel agents diff         # what would change without writing
```

`keel install` runs an `apply-agents` synthetic step before
`install-hooks` automatically when this section is present
(`[agents].install_with_setup` defaults to true).

## Local overrides

Per the whole-file-ownership rule, keel never edits files it
...
- For `.claude/skills/` (a `[[dir]]` target) — local skills coexist
  in the same directory; keel tracks ownership in
  `.keel/agents.state.json` and only updates the files it owns.
