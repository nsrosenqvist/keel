...
shared agent instructions and skills via `ampelos agents`.

## Layout

```
ampelos-agents.toml      # the manifest ampelos reads downstream
...
```sh
ampelos agents install      # pull pinned upstream into the project
ampelos agents update       # re-resolve revs (auto-refetches floating refs)
ampelos agents status       # per-source pinned rev + per-file drift
ampelos agents diff         # what would change without writing
```

`ampelos install` runs an `apply-agents` synthetic step before
`install-hooks` automatically when this section is present
(`[agents].install_with_setup` defaults to true).

## Local overrides

Per the whole-file-ownership rule, ampelos never edits files it
...
- For `.claude/skills/` (a `[[dir]]` target) — local skills coexist
  in the same directory; ampelos tracks ownership in
  `.ampelos/agents.state.json` and only updates the files it owns.
