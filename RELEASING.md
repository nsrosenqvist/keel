...
- [ ] On the dev machine, `cargo build --release --bin croft` succeeds
      and `env!("CROFT_TARGET")` resolves to a known target triple.
...
- [ ] From the freshly built binary:
  ...
  ```sh
  croft update                # → "already on the latest version"
  croft update --prerelease   # → upgrades to v0.0.2-rc.1
  ```
...
The `publish` job runs `cargo publish` against the single `croft`
...
- [ ] Generate a crates.io API token with **publish-new** +
      **publish-update** scope on the `croft` crate (or unrestricted).
...
The `update-homebrew` job checks out `nsrosenqvist/homebrew-croft`,
patches `Formula/croft.rb` with the new version + four sha256 sums,
and pushes a single commit per release.

- [ ] Create the public repo `nsrosenqvist/homebrew-croft`.

- [ ] Seed `Formula/croft.rb` with **four** `sha256 "PLACEHOLDER"`
  ...
  class Croft < Formula
    ...
    test do
      assert_match "croft", shell_output("#{bin}/croft --version")
    end
  end
  ```

- [ ] Create a fine-grained PAT with `Contents: Read and write` on the
      tap repo (only). Add it as a repo secret on the croft repo named
      `HOMEBREW_TAP_TOKEN`.
...
- crates.io: `croft` shows the new version.
- Homebrew tap: a new commit on `main` titled `croft vX.Y.Z`.
- Floating tags: `git ls-remote --tags origin` shows `vX` and `vX.Y`
  pointing at the same commit as `vX.Y.Z`.
- `croft update` (without `--prerelease`) on a binary from the previous
