...
use crate::container::devcontainer::{DevcontainerBackend, DevcontainerIdentity, DevcontainerSpec};
use crate::container::{Backend, compose::ComposeBackend};
use crate::runtime::{Executor, Resolution, Resolver, ResolverContext};
...
pub async fn run(cli: Cli) -> Result<()> {
    ...
    let config = crate::config::load_project_with_slug(&project_root, slug_for_overlay)
        .with_context(|| format!("load project at {}", project_root.display()))?;
    ...
    if let Some(sub) = cli.command {
        return match sub {
            ...
            Command::Shell { service } => {
                let code =
                    commands::shell::run(&cfg_arc, &project_root, &identity, service.as_deref())
                        .await?;
                std::process::exit(code);
            }
            Command::Install {
...
fn ensure_ampelos_gitignore(config: &Config, project_root: &Path) -> Result<()> {
    ...
    let body = "local.toml\nworktrees/\ncache/\ninstall.state.json\nagents.state.json\n";
    crate::config::managed_block::write(&path, body)
...
fn project_slug_from_root(project_root: &Path) -> String {
    let basename = project_root
        ...
        .and_then(|s| s.to_str())
        .unwrap_or("project");
    crate::runtime::slugify(basename)
}
...
async fn build_backend(config: &Config) -> Result<Arc<dyn Backend>> {
    ...
    for svc in &config.services.custom {
        entries.push(crate::runtime::services::from_custom(svc));
    }
    for svc in &config.services.systemd {
        entries.push(crate::runtime::services::from_systemd(svc));
    }
    let custom = if entries.is_empty() {
        None
    } else {
        Some(crate::container::custom::CustomBackend::new(entries))
    };

    Ok(Arc::new(crate::container::registry::ServiceRegistry::new(
        container, custom,
    )))
}
...
async fn run_tui(
    ...
    let mut next_view = crate::tui::View::ControlCenter;
    loop {
        let backend: Arc<dyn Backend> = match build_backend(&current_config).await {
            Ok(b) => b,
            Err(_) => Arc::new(crate::container::registry::ServiceRegistry::new(
                Some(Arc::new(crate::container::null::NullBackend)),
                None,
            )),
        };
        // Re-detect identity per loop iteration — worktree switches
        // change which slug the devcontainer container_name embeds.
        let identity = crate::runtime::Identity::detect(&current_root, &current_config).await;
        ...
        .context("run TUI")?;
        match outcome {
            crate::tui::DriveOutcome::Quit => return Ok(()),
            crate::tui::DriveOutcome::SwitchWorktree { path, view } => {
                ...
                let bootstrap_cfg = crate::config::load_project_with_slug(&path, None)
                    .with_context(|| format!("load project at {}", path.display()))?;
                let identity = crate::runtime::Identity::detect(&path, &bootstrap_cfg).await;
                let slug_for_overlay = if identity.is_isolated() {
                    Some(identity.slug.as_str())
                } else {
                    None
                };
                let new_cfg = crate::config::load_project_with_slug(&path, slug_for_overlay)
                    .with_context(|| format!("load project at {}", path.display()))?;
                current_root = path;
...
fn branch_label(identity: &crate::runtime::Identity) -> Option<String> {
    match &identity.base_ref {
        crate::runtime::BaseRef::Branch(b) => Some(b.clone()),
        crate::runtime::BaseRef::DetachedSha(s) => Some(format!("det-{s}")),
        crate::runtime::BaseRef::WorktreeDir(d) => Some(d.clone()),
        crate::runtime::BaseRef::None => None,
    }
}
