...
async fn check_custom_services(config: &Config) -> Vec<Finding> {
    use crate::runtime::services::{from_custom, from_systemd};
...
async fn check_worktree(config: &Config, project_root: &Path) -> Vec<Finding> {
    let identity = crate::runtime::Identity::detect(project_root, config).await;
...
async fn check_devcontainer(config: &Config, project_root: &Path) -> Vec<Finding> {
    use crate::container::devcontainer::{ContainerSource, DevcontainerSpec};
    ...
    let mut out = Vec::new();
    let path = match DevcontainerSpec::discover(project_root, config.devcontainer.path.as_deref()) {
        Ok(p) => p,
        Err(e) => {
    ...
    let identity = crate::runtime::Identity::detect(project_root, config).await;
    let dc = match crate::cli::app::build_devcontainer(config, project_root, &identity) {
        Ok(Some(b)) => b,
