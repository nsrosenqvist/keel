//! `croft init` — scaffold a starter `croft.toml`.
//!
...
pub fn run(project_root: &Path, template: Option<Template>) -> Result<()> {
    let target = project_root.join("croft.toml");
    if target.exists() {
...
mod tests {
    ...
    fn writes_template_when_absent() {
        ...
        let body = std::fs::read_to_string(dir.path().join("croft.toml")).unwrap();
    ...
    fn refuses_to_overwrite() {
        let dir = TempDir::new().unwrap();
        std::fs::write(dir.path().join("croft.toml"), "existing").unwrap();
        let err = run(dir.path(), None).unwrap_err();
        assert!(err.to_string().contains("already exists"));
        assert_eq!(
            std::fs::read_to_string(dir.path().join("croft.toml")).unwrap(),
            "existing"
        );
    }
