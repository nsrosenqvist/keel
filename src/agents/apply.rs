...
//!
//! `apply()` is the single entry point used by both `croft agents
//! install` / `update` and the synthetic step inside `croft install`.
...
use crate::agents::manifest::{FileMode, parse_manifest};
use crate::agents::merge::{ResolvedEntry, expand_source};
use crate::agents::state::{AgentsState, AppliedFile, SourceRecord, epoch_ms};
use crate::cache::{CacheKind, RepoRef};
use crate::config::{AgentsConfig, SourceSpec};
...
pub struct ApplyOptions {
    ...
    pub dry_run: bool,
    /// Overwrite croft-owned files that have been hand-edited since
...
/// One destination edited by hand since croft last wrote it.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub async fn apply(
    ...
    for source in &active_sources {
        ...
        let cached = crate::cache::clone_or_reuse(
            project_root,
            &RepoRef {
...
/// Drift = croft-owned file whose disk content hashes to something
