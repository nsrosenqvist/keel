...
use crate::config::install::discover_install_steps;
...
/// Read a `croft.toml` (or other TOML file) from disk and parse it.
...
/// 3. `.croft/worktrees/<slug>.toml` when `slug` is `Some` and the
///    file exists (per-worktree overrides; gitignored).
///
/// Missing `croft.toml` is fine — a default config is used as the
/// base. Plus: scripts under `.croft/commands/` are discovered after
/// merging.
///
...
pub fn load_project_with_slug(
    ...
    let toml_path = project_root.join("croft.toml");
    let local_path = project_root.join(".croft").join("local.toml");
    ...
    let scripts_dir = project_root.join(".croft").join("commands");
    if scripts_dir.is_dir() {
        config.scripts = discover_scripts(&scripts_dir)?;
    }
    let install_dir = project_root.join(".croft").join("install");
...
mod tests {
    ...
    fn load_from_path_reads_file() {
        ...
        let path = dir.path().join("croft.toml");
        std::fs::write(
            &path,
            r#"
                [project]
    ...
    fn load_project_discovers_scripts() {
        ...
        .unwrap();
        let cmds = root.path().join(".croft").join("commands");
    ...
    fn load_project_merges_local_overlay() {
        ...
        .unwrap();
        std::fs::create_dir_all(root.path().join(".croft")).unwrap();
        std::fs::write(
            root.path().join(".croft").join("local.toml"),
            r#"
                [project]
                name = "overridden"
    ...
    fn load_project_with_slug_applies_overlay() {
        ...
        .unwrap();
        std::fs::create_dir_all(root.path().join(".croft").join("worktrees")).unwrap();
        std::fs::write(
            root.path()
                .join(".croft")
                .join("worktrees")
    ...
    fn load_project_with_slug_layers_after_local() {
        ...
        .unwrap();
        std::fs::write(
            root.path().join(".croft").join("worktrees").join("x.toml"),
            r#"
                [project]
                name = "from-worktree"
            "#,
