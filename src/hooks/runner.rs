...
//! 2. **Cached upstream repo** (`repo: <url>` with a `rev`): croft
//!    clones the repo into `.croft/cache/hooks/<key>/` and reads its
//!    `.pre-commit-hooks.yaml` to fill in any fields the user didn't
//!    override. Same execution path after merging.
//!
//! The hook's `language` tag is **advisory**: croft runs the entry
//! verbatim regardless of `python` / `node` / `ruby` / … and trusts
//! the runtime to be on `PATH` (typically installed by `croft install`).
...
//! implement. croft does not bridge to the `pre-commit` binary;
...
mod tests {
    ...
    async fn local_python_hook_runs_verbatim_when_entry_on_path() {
        // The language tag is advisory now — croft doesn't gate on it.
