...
//! - [`cache`] clones external hook repos into `.croft/cache/hooks/`
//!   and remembers the resolved revision sha. croft owns this cache
//!   end-to-end; it never delegates to the `pre-commit` binary.
//! - [`runner`] runs the hooks for a given stage. Each hook's `entry`
//!   is exec'd verbatim regardless of the declared `language` — croft
//!   trusts the user to have the runtime on `PATH` (typically via
//!   `croft install`) rather than reimplementing pre-commit's
...
//!   `croft hooks run <stage> "$@"`.
//!
//! Native croft hooks (declared in `croft.toml` `[hooks]`) are run by
...
pub use installer::{CROFT_HOOK_MARKER, install, install_one, uninstall};
