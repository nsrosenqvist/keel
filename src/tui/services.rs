...
//!
//! When a project's `[[ui.pane]]` declares a service, keel spawns a
...
//! [`crate::tui::worker`] on its own cadence; the worker pushes
//! `ServiceStatus` snapshots that the render loop folds into
//! `ServicePane::status` via `App::drain_worker_snapshots`.

use crate::container::{Backend, BackendError, ServiceStatus};
use crate::runtime::{OutputLine, OutputStream};
use crate::tui::runner::{CapturedLine, OUTPUT_BUFFER_CAP, push_capped};
