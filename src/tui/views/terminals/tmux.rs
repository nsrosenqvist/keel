...
//!
//! Everything croft knows about tmux at the process level lives here:
...
/// croft-specific options (status bar, detach binding, bell
...
pub(crate) async fn attach_tmux(req: &AttachRequest) {
    ...
    let target = if req.window == "croft-new" {
    ...
    if !req.window_options.is_empty() {
        ...
        let win_target = if req.window == "croft-new" {
            format!("{}:", req.session)
        } else {
            format!("{}:{}", req.session, req.window)
        };
    ...
    // moment croft detached — definitely not what "detach"
    ...
    // outside croft) in sync. Nothing global on the server.
    ...
    // Inject the croft-flavoured status bar so users always see
...
pub async fn list_tmux_windows(session: &str) -> WindowList {
    ...
    let output = match Command::new("tmux")
        .args([
            ...
            session,
            "-F",
            "#{window_index}\t#{window_name}\t#{pane_current_path}\t#{window_bell_flag}\t#{@croft_workspace}",
        ])
...
/// cleared. The `@croft_workspace` column is empty for any window
/// croft didn't tag (host shells, services, pre-existing windows).
/// Public for tests.
