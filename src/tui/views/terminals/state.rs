...
pub struct TmuxWindow {
    ...
    pub has_bell: bool,
    /// Devcontainer workspace folder, read from the `@croft_workspace`
    /// tmux window option croft set when it created the window.
    /// Empty for windows croft didn't tag (host shells, service
...
pub struct TerminalsView {
    ...
    /// `croft-<project>-<slug>` — stable per worktree.
    ...
    /// croft to beep again on its way back to the TUI.
...
impl TerminalsView {
    ...
    pub fn default_for(config: &Config) -> Self {
        ...
        Self {
            tmux_available: None,
            session_name: format!("croft-{project}"),
            windows: Vec::new(),
