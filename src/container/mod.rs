...
pub trait Backend: Send + Sync {
    ...
    /// execution: croft pipes the script body into `<interpreter> -s
    ...
    /// `OutputSink` in `croft-runtime`) and awaits exit. Used by the
    ...
    ///
    /// [`OutputSink`]: croft-runtime::OutputSink
    async fn tail_logs(&self, _service: &str) -> Result<Child, BackendError> {
        Err(BackendError::Reported(
            "no container backend configured (set runtime.backend = \"compose\" in croft.toml)"
                .into(),
        ))
    }
