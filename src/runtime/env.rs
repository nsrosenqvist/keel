...
///     (dotenv files, CROFT_* injection, `[env]`, recipe / script
...
impl Env {
    ...
    pub async fn resolve_with_identity(
        ...
        // entries can reference CROFT_WORKTREE_OFFSET via `offset`.
    ...
    /// script's `env = {...}`, or CROFT_*-prefixed executor injection) —
    ...
    /// command resolution. Keys come from dotenv files, CROFT_*
    ...
    /// truth for the rule "croft-spawned host processes see exactly
...
/// Inject `CROFT_WORKTREE_*` and (when isolation is on)
...
/// was kept from existing env) because croft owns the semantics of
/// these names.
fn inject_worktree_env(
    ...
    // offset 0.
    vars.entry("CROFT_WORKTREE_SLUG".into())
        .or_insert_with(|| identity.slug.clone());
    project_keys.insert("CROFT_WORKTREE_SLUG".into());
    vars.entry("CROFT_WORKTREE_OFFSET".into())
        .or_insert_with(|| identity.offset.to_string());
    project_keys.insert("CROFT_WORKTREE_OFFSET".into());
...
mod tests {
    ...
    async fn required_missing_errors() {
        ...
        unsafe {
            std::env::remove_var("CROFT_TEST_REQ_MISSING");
        }
        let err = Env::resolve(&cfg, std::env::current_dir().unwrap().as_path())
            .await
            .unwrap_err();
        match err {
            RuntimeError::RequiredEnvMissing(name) => {
                assert_eq!(name, "CROFT_TEST_REQ_MISSING")
            }
            other => panic!("unexpected: {other:?}"),
        }
    }
    ...
    async fn explicit_value_wins_over_existing() {
        unsafe {
            std::env::set_var("CROFT_TEST_VAR", "from-process");
        }
        let cfg = cfg_with_env(&[(
            "CROFT_TEST_VAR",
            EnvSpec {
        ...
        let env = Env::resolve(&cfg, std::env::current_dir().unwrap().as_path())
            .await
            .unwrap();
        assert_eq!(env.get("CROFT_TEST_VAR"), Some("from-config"));
        unsafe {
            std::env::remove_var("CROFT_TEST_VAR");
        }
    }
    ...
    async fn apply_to_clears_then_injects() {
        ...
        unsafe {
            std::env::set_var("CROFT_APPLY_TO_MARKER", "leaked");
        }
        ...
        cmd.arg("-c")
            .arg(r#"echo FOO=$FOO; echo BAZ=$BAZ; echo MARKER=${CROFT_APPLY_TO_MARKER:-<unset>}"#);
        env.apply_to(&mut cmd);

        let out = cmd.output().await.unwrap();
        let text = String::from_utf8(out.stdout).unwrap();
        unsafe {
            std::env::remove_var("CROFT_APPLY_TO_MARKER");
        }
    ...
    async fn project_only_map_excludes_inherited_host_env() {
        ...
        unsafe {
            std::env::set_var("CROFT_HOST_LEAK", "from-shell");
        }
        let cfg = cfg_with_env(&[(
            "CROFT_PROJECT_VAR",
            EnvSpec {
        ...
        let env = Env::resolve(&cfg, std::env::current_dir().unwrap().as_path())
            ...
            .unwrap();
        unsafe {
            std::env::remove_var("CROFT_HOST_LEAK");
        }
        let project = env.project_only_map();
        assert_eq!(
            project.get("CROFT_PROJECT_VAR").map(String::as_str),
            Some("from-config")
        );
        assert!(
            !project.contains_key("CROFT_HOST_LEAK"),
        ...
        // CROFT_WORKTREE_* are project-declared even though their value
        // came from the executor's identity injection.
        assert!(project.contains_key("CROFT_WORKTREE_SLUG"));
        assert!(project.contains_key("CROFT_WORKTREE_OFFSET"));
    }
    ...
    fn inject_worktree_env_sets_slug_and_offset() {
        ...
        assert_eq!(
            vars.get("CROFT_WORKTREE_SLUG").map(String::as_str),
            Some("feature-x")
        );
        assert_eq!(
            vars.get("CROFT_WORKTREE_OFFSET").map(String::as_str),
            Some("42")
        );
        assert_eq!(
        ...
        assert!(keys.contains("CROFT_WORKTREE_SLUG"));
        assert!(keys.contains("CROFT_WORKTREE_OFFSET"));
    ...
    fn inject_worktree_env_keeps_existing_compose_name() {
        ...
        // croft owns the semantics of the name, so it must survive the
