//! `scaffl shell` — drop into an interactive shell.
//!
//! Two flavours:
//!
...
/// Entry point for `scaffl shell` and `scaffl shell --service <name>`.
pub async fn run(
    config: &Config,
    project_root: &Path,
...
async fn shell_into_devcontainer(
    ...
    if !config.devcontainer.enabled {
        anyhow::bail!(
            "no shell target. Either enable `[devcontainer] enabled = true` in scaffl.toml, \
             or pass `--service <name>` to enter a compose service."
        );
    }
    let dc = crate::app::build_devcontainer(config, project_root, identity)
