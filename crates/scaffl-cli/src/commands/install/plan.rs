//! Turn `[install]` config plus `.scaffl/install/*` into an ordered,
...
//!    don't have to touch `scaffl.toml` to wire them up.
//! 2. When `[install].steps` is set, every named entry must resolve
//!    against `.scaffl/install/`, a `[command.*]` recipe, or — by
//!    explicit opt-in — `.scaffl/commands/`. Unresolved names are a
//!    fatal config error.
...
use scaffl_config::{Config, InlineStep, InstallStepRef, InstallStepScript};
...
#[derive(Debug, Clone)]
pub enum StepSource {
    /// Execute via [`scaffl_runtime::Executor::run_recipe`].
    Recipe(String),
    /// Execute a script file under `.scaffl/install/` directly.
...
fn resolve_entry(config: &Config, entry: &InstallStepRef) -> Result<Step> {
    match entry {
        ...
        InstallStepRef::Name(name) => {
            ...
            // We deliberately do *not* fall back to `.scaffl/commands/`
            // scripts — those are user-facing recipes that show up in
            // `scaffl list`, and we want install authors to be
            // explicit about wiring one into the install flow.
            bail!(
                "install step `{}` does not match a discovered .scaffl/install/ file \
...
mod tests {
    use super::*;
    use scaffl_config::{Config, InstallStepScript};
