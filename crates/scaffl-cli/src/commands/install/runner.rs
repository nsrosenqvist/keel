...
use scaffl_config::{Config, InlineStep, InstallStepScript};
use scaffl_container::Backend;
use scaffl_runtime::{ChannelSink, Env, Executor};
...
/// (`scaffl-cli/src/app.rs`) propagates it via `std::process::exit`.
pub async fn run(
    config: Arc<Config>,
    ...
    let project_label = config
        ...
        .name
        .clone()
        .unwrap_or_else(|| "scaffl".to_string());
...
fn decide_start_index(plan: &[Step], state: &InstallState, args: &InstallArgs) -> Result<usize> {
    ...
    // Bare `scaffl install`: if there's any unresolved step from a
...
async fn run_single(
    ...
    let (idx, step) = plan
        ...
        .with_context(|| {
            format!("no install step named `{name}`. Run `scaffl install --list` to see the plan.")
        })?;
    let project_label = config
        .project
        .name
    ...
    // bare `scaffl install` sees this step as resolved.
...
/// terminal so `scaffl lib ask` & friends Just Work. No output is
...
async fn run_script(
    ...
    cmd.env("SCAFFL_PROJECT_DIR", project_root.display().to_string());
    if let Some(parent) = script.path.parent() {
        cmd.env("SCAFFL_SCRIPT_DIR", parent.display().to_string());
    }
...
async fn run_install_hooks(
    ...
    if pre_commit_path.is_file() {
        match scaffl_hooks::config::load_from_path(&pre_commit_path) {
            Ok(pcconf) => {
                for repo in &pcconf.repos {
                    ...
                    if repo.is_meta() {
                        renderer.append_tail(&format!(
                            "skipping `repo: meta` ({} hooks) — not supported by scaffl",
                            repo.hooks.len()
                        ));
                        continue;
                    }
                    ...
                    if let Err(e) =
                        scaffl_hooks::cache::clone_or_reuse(project_root, repo, force_update).await
                    {
    ...
    let installed =
        scaffl_hooks::installer::install(project_root, &stages).context("install hook shims")?;
