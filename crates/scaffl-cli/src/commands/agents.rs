//! `scaffl agents` subcommand: manage upstream-sourced agent
//! instructions and skills.
//!
//! Thin CLI wrapper around `scaffl_agents::apply` / `detect_drift`.
//! All policy lives in the `scaffl-agents` crate; this module
//! formats arguments + report output.

use anyhow::{Context, Result};
use comfy_table::{ContentArrangement, Table, presets::UTF8_FULL};
use scaffl_agents::{ApplyOptions, ApplyReport, apply, detect_drift};
use scaffl_config::Config;
use std::path::Path;

/// Run `scaffl agents install`. Mirrors `scaffl agents update` when
...
/// Run `scaffl agents update`. Same pipeline as install, plus an
/// optional source filter and an implicit re-fetch for floating refs
/// (handled inside `scaffl_agents::apply`).
pub async fn update(
    config: &Config,
    project_root: &Path,
    sources: Vec<String>,
...
/// Run `scaffl agents diff` — equivalent to apply with `dry_run = true`.
...
/// Run `scaffl agents status`. Read-only — never touches the cache or
...
pub async fn status(config: &Config, project_root: &Path, strict: bool) -> Result<i32> {
    let state = scaffl_agents::AgentsState::load(project_root)?.unwrap_or_default();
