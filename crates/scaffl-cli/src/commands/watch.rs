//! `scaffl watch <recipe>` — re-run a recipe when watched files change.
//!
...
//! the recipe through [`scaffl_runtime::Executor`] (inheriting stdio so the
...
const IGNORED_SEGMENTS: &[&str] = &["target", ".git", "node_modules", ".scaffl"];
