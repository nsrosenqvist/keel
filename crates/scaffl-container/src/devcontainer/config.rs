...
pub enum DevcontainerConfigError {
    ...
    #[error(
        "{path}: `dockerComposeFile` devcontainers are not supported in scaffl v1. \
         Use `containers.backend = \"compose\"` in scaffl.toml and route recipes \
...
/// The parsed devcontainer descriptor, normalised into scaffl's
...
pub struct DevcontainerSpec {
    ...
    /// Optional human-readable name. Not used by scaffl for
    ...
    /// scaffl's recipe env (recipe env wins on conflict).
...
/// json5 layer doesn't need to know about scaffl conventions.
...
mod tests {
    ...
    fn parses_build_with_defaults() {
        ...
        let spec = DevcontainerSpec::from_str(&path(".devcontainer/devcontainer.json"), src)
            .unwrap();
        match spec.source {
            ContainerSource::Build {
                dockerfile,
    ...
    fn parses_full_spec_subset() {
        ...
        assert_eq!(spec.container_env.get("TZ").map(String::as_str), Some("UTC"));
        assert_eq!(spec.remote_env.get("EDITOR").map(String::as_str), Some("vim"));
    ...
    fn discover_missing_override_errors_with_path() {
        let tmp = tempfile::tempdir().unwrap();
        let err =
            DevcontainerSpec::discover(tmp.path(), Some("nope.json")).expect_err("must fail");
