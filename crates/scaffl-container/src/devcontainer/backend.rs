...
use crate::devcontainer::config::{ContainerSource, DevcontainerSpec};
...
/// `scaffl_runtime::worktree::Identity`) and hand them in.
#[derive(Debug, Clone)]
pub struct DevcontainerIdentity {
    /// Absolute path to the project (or worktree) root. Becomes the
    /// bind-mount source and the `scaffl.devcontainer.root` label
    /// value.
...
/// What [`ensure_up`] will do. Exposed for diagnostics (`scaffl
...
impl DevcontainerBackend {
    ...
    /// calls); used by `scaffl doctor` to summarise state.
    ///
    /// [`ensure_up`]: Self::ensure_up
    ...
    async fn run_container(&self) -> Result<(), DevcontainerError> {
        ...
        if !self.identity.worktree_slug.is_empty() {
            cmd.args([
                "--label",
                &format!("scaffl.devcontainer.worktree={}", self.identity.worktree_slug),
            ]);
        }
...
/// Container name pattern: `scaffl-devcontainer-<project-slug>[-<worktree-slug>]`.
/// Docker permits `[a-zA-Z0-9][a-zA-Z0-9_.-]*` so the slug rules from
/// `scaffl-runtime::worktree::slugify` (lowercase alnum + `-`) compose
/// cleanly.
fn derive_container_name(identity: &DevcontainerIdentity) -> String {
    ...
    if identity.worktree_slug.is_empty() {
        format!("scaffl-devcontainer-{project}")
    } else {
        format!(
            "scaffl-devcontainer-{project}-{}",
            identity.worktree_slug
        )
    }
}
...
/// `scaffl-devcontainer-<slug>:<sha256(dockerfile+args)[..12]>` for
...
fn derive_image_ref(spec: &DevcontainerSpec, identity: &DevcontainerIdentity) -> String {
    match &spec.source {
        ...
        ContainerSource::Build {
            ...
            format!("scaffl-devcontainer-{project}:{tag}")
        }
    }
}
...
mod tests {
    ...
    fn container_name_includes_worktree_slug_when_isolated() {
        let backend = DevcontainerBackend::new(spec_image(), ident("scaffl", "feature-x"));
        assert_eq!(backend.container_name(), "scaffl-devcontainer-scaffl-feature-x");
    }

    #[test]
    fn container_name_omits_worktree_when_empty() {
        let backend = DevcontainerBackend::new(spec_image(), ident("scaffl", ""));
        assert_eq!(backend.container_name(), "scaffl-devcontainer-scaffl");
    }

    #[test]
    fn container_name_falls_back_when_project_slug_empty() {
        let backend = DevcontainerBackend::new(spec_image(), ident("", ""));
        assert_eq!(backend.container_name(), "scaffl-devcontainer-project");
    }
    ...
    fn workspace_folder_uses_spec_value_when_present() {
        ...
        .unwrap();
        let backend = DevcontainerBackend::new(spec, ident("scaffl", ""));
    ...
    fn image_ref_hashes_when_building() {
        ...
        assert!(
            image_ref.starts_with("scaffl-devcontainer-scaffl:"),
            "got: {image_ref}"
        );
        let tag = image_ref
            .split_once(':')
    ...
    fn plan_reflects_build_source() {
        ...
        assert_eq!(plan.container_name, "scaffl-devcontainer-scaffl");
    }
