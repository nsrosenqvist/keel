...
use crate::commands::install::plan::{Step, StepSource, resolve_cwd};
use crate::commands::install::renderer::{Renderer, StepOutcome};
use crate::commands::install::state::{InstallState, StepStatus, epoch_ms};
use anyhow::{Context, Result};
use keel_config::{Config, InlineStep, InstallStepScript};
use keel_container::Backend;
use keel_runtime::{ChannelSink, Env, Executor};
...
/// (`keel-cli/src/app.rs`) propagates it via `std::process::exit`.
pub async fn run(
    config: Arc<Config>,
...
fn decide_start_index(plan: &[Step], state: &InstallState, args: &InstallArgs) -> Result<usize> {
    ...
    // Bare `keel install`: if there's any unresolved step from a
...
async fn run_single(
    ...
    let (idx, step) = plan
        ...
        .with_context(|| {
            format!("no install step named `{name}`. Run `keel install --list` to see the plan.")
        })?;
    let project_label = config
        .project
        .name
    ...
    // bare `keel install` sees this step as resolved.
...
/// terminal so `keel lib ask` & friends Just Work. No output is
...
async fn run_script(
    ...
    cmd.env("KEEL_PROJECT_DIR", project_root.display().to_string());
    if let Some(parent) = script.path.parent() {
        cmd.env("KEEL_SCRIPT_DIR", parent.display().to_string());
    }
...
async fn run_install_hooks(
    ...
    if pre_commit_path.is_file() {
        match keel_hooks::config::load_from_path(&pre_commit_path) {
            Ok(pcconf) => {
                for repo in &pcconf.repos {
                    ...
                    if repo.is_meta() {
                        renderer.append_tail(&format!(
                            "skipping `repo: meta` ({} hooks) — not supported by keel",
                            repo.hooks.len()
                        ));
                        continue;
                    }
                    ...
                    if let Err(e) =
                        keel_hooks::cache::clone_or_reuse(project_root, repo, force_update).await
                    {
    ...
    let installed =
        keel_hooks::installer::install(project_root, &stages).context("install hook shims")?;
...
async fn run_apply_agents(
    ...
    let opts = keel_agents::ApplyOptions::default();
    let report = match keel_agents::apply(project_root, &config.agents, &opts).await {
        Ok(r) => r,
        Err(e) => {
...
/// rules so the install flow and the explicit `keel hooks install`
/// flow agree.
