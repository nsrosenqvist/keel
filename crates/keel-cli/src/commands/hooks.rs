...
//!   these run as recipes through [`keel_runtime::Executor`].
...
use keel_hooks::{HookOutcome, installer};
...
/// Default stages for `keel hooks install` (no `--stages`):
...
pub async fn uninstall(project_root: &Path, stages: &[String]) -> Result<()> {
    ...
    if removed.is_empty() {
        println!("No keel-managed hooks to remove.");
    } else {
        for path in removed {
...
pub async fn run(config: &Arc<Config>, project_root: &Path, stage: &str) -> Result<i32> {
    ...
    if !native_hooks.is_empty() {
        ...
        for name in native_hooks {
            ...
            if code != 0 {
                eprintln!("[keel] {name}: failed (exit {code})");
    ...
    if pcc_path.exists() && overall == 0 {
        let pcc = keel_hooks::config::load_from_path(&pcc_path)
            .with_context(|| format!("load {}", pcc_path.display()))?;
        let outcomes = keel_hooks::run_pre_commit(&pcc, project_root, stage)
            .await
    ...
    if native_hooks.is_empty() && !pcc_path.exists() {
        ...
        if dotenv_stage_handled(config, stage) {
            println!("[keel] refreshed dotenv for stage `{stage}`");
        } else {
...
async fn build_backend_or_null(config: &Config) -> Arc<dyn Backend> {
    ...
    match config.containers.backend {
        ...
        _ => Arc::new(keel_container::null::NullBackend),
    }
}
