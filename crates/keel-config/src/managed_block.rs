//! Marker-delimited "keel-managed" block writer.
//!
//! Several keel features write structured content into files the user
...
pub const BEGIN_MARKER: &str = "# >>> keel-managed (auto-generated; do not edit by hand) >>>";

/// Closing marker line.
pub const END_MARKER: &str = "# <<< keel-managed <<<";

const BEGIN_PREFIX: &str = "# >>> keel-managed";
const END_PREFIX: &str = "# <<< keel-managed";

/// Idempotent write: replace (or insert) the keel-managed block in
...
/// Drop the keel-managed block (markers and every line between) from
