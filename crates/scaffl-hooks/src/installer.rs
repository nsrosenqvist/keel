...
//! `scaffl hooks run <stage> "$@"`. Each shim contains a marker comment so
...
/// Write a shim for a single stage. Refuses to overwrite a non-scaffl
/// hook.
pub fn install_one(hooks_dir: &Path, stage: &str) -> Result<PathBuf, HookError> {
    ...
    if path.exists() {
        ...
        if !existing.contains(SCAFFL_HOOK_MARKER) {
...
/// Remove scaffl-managed shims from `stages`. Untouched: hooks we didn't
...
pub fn uninstall(project_root: &Path, stages: &[&str]) -> Result<Vec<PathBuf>, HookError> {
    ...
    for stage in stages {
        ...
        if !body.contains(SCAFFL_HOOK_MARKER) {
            continue;
        }
...
mod tests {
    ...
    fn install_writes_shim() {
        ...
        assert!(body.contains(SCAFFL_HOOK_MARKER));
        assert!(body.contains("scaffl hooks run pre-commit"));
    }
