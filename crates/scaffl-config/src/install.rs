//! `[install]` configuration and `.scaffl/install/` step discovery.
//!
...
//! - **Discovered** — shell files under `.scaffl/install/` are picked up
...
//! `scaffl list` or the TUI sidebar. The author can still reference a
...
pub struct InstallConfig {
    ...
    /// is the alphabetical listing of `.scaffl/install/*` instead.
    #[serde(default)]
    ...
    /// Set `false` if the project doesn't want hooks managed by scaffl.
    ...
    /// inside `.scaffl/` so it only governs scaffl-owned files and
    /// can't accidentally shadow the project's own root `.gitignore`.
    #[serde(default = "default_gitignore")]
    pub gitignore: String,

    /// Steps discovered under `.scaffl/install/`. Populated by the
...
pub struct InlineStep {
    ...
    /// Required so a failed step can be referenced by `scaffl install
    /// <name>` on retry.
    pub name: String,

    /// Shell command to run. Single string form only; multi-step
    /// inline blocks belong in a `.scaffl/install/<name>` file or a
    /// regular recipe.
    pub run: String,
...
/// A step discovered under `.scaffl/install/`. Same frontmatter parser
...
/// Scan `.scaffl/install/` for step files. Same skip rules as the
/// `.scaffl/commands/` scan (hidden / underscore-prefixed files are
...
mod tests {
    ...
    fn install_section_defaults_when_absent() {
        ...
        assert_eq!(cfg.install.gitignore, ".scaffl/.gitignore");
