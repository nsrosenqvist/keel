...
use keel_config::{Config, Recipe, ScriptCommand, model::UiPane};
...
pub struct NewWorktreeForm {
    ...
    pub branches: Vec<keel_runtime::BranchEntry>,
...
fn sync_path_from_branch(form: &mut NewWorktreeForm) {
    ...
    let dir = if form.branch_input.is_empty() {
        ...
        keel_runtime::slugify(&form.branch_input)
    };
    if dir.is_empty() {
...
impl App {
    ...
    /// runs `keel_runtime::list_branches` first (async) and hands
    ...
    pub fn open_create_form(
        &mut self,
        branches: Vec<keel_runtime::BranchEntry>,
    ...
    pub fn spawn_boot_tasks(&mut self, project_root: &Path) {
        ...
        tokio::spawn(async move {
            let trunk =
                keel_runtime::detect_trunk(&project_root_owned, configured_base.as_deref()).await;
            let anchor = match trunk.as_deref() {
                Some(t) => keel_runtime::merge_base(&project_root_owned, t).await,
                None => None,
            };
...
mod tests {
    ...
    fn launch_rejects_service_selection() {
        ...
        let backend: Arc<dyn keel_container::Backend> =
            Arc::new(keel_container::null::NullBackend);
