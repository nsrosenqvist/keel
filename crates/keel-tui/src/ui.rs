...
use keel_container::ServiceStatus;
use keel_runtime::OutputStream;
...
fn recipe_kv_lines(recipe: &keel_config::Recipe) -> Vec<Line<'static>> {
    let mut out = Vec::new();
...
fn script_kv_lines(script: &keel_config::ScriptCommand) -> Vec<Line<'static>> {
    let mut out = Vec::new();
...
mod tests {
    ...
    fn renders_empty_config() {
        ...
        let mut terminal = Terminal::new(backend).unwrap();
        let app = App::new(Arc::new(keel_config::Config::default()));
        terminal.draw(|f| render(&app, f)).unwrap();
    }
