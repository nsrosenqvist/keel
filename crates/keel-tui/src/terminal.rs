...
///   - `req.window == "keel-new"` (the sentinel marker) → spawn
    ...
    match code {
        ...
        KeyCode::Char('s') => {
            ...
            if app.abort_selected_run() {
                ...
                && let Err(rej) = app
                    .run_service_action(keel_container::service_action::STOP, &[service.as_str()])
                    .await
            {
        ...
        KeyCode::Char('S') => {
            if app.abort_lifecycle_run() {
                ...
                .run_service_action(keel_container::service_action::STOP, &[])
                .await
            {
                app.flash = Some(launch_message(rej));
            }
        ...
        KeyCode::Char('r') => {
            if let Some(service) = app.selected_service().map(|s| s.name.clone())
                && let Err(rej) = app
                    .run_service_action(
                        keel_container::service_action::RESTART,
                        &[service.as_str()],
                    )
                    .await
            {
        ...
        KeyCode::Char('R') => {
            if let Err(rej) = app
                .run_service_action(keel_container::service_action::RESTART, &[])
                .await
            {
                app.flash = Some(launch_message(rej));
            }
        ...
        KeyCode::Char('u') => {
            if let Some(service) = app.selected_service().map(|s| s.name.clone())
                && let Err(rej) = app
                    .run_service_action(keel_container::service_action::UP, &[service.as_str()])
                    .await
            {
                app.flash = Some(launch_message(rej));
            }
        ...
        KeyCode::Char('U') => {
            if let Err(rej) = app
                .run_service_action(keel_container::service_action::UP, &[])
                .await
            {
                app.flash = Some(launch_message(rej));
            }
        ...
        KeyCode::Char('D') => {
            if let Err(rej) = app
                .run_service_action(keel_container::service_action::DOWN, &[])
                .await
            {
                app.flash = Some(launch_message(rej));
            }
...
async fn refresh_diff_anchor(app: &mut App) {
    ...
    let trunk = keel_runtime::detect_trunk(&project_root, configured.as_deref()).await;
    let anchor = match trunk.as_deref() {
        Some(t) => keel_runtime::merge_base(&project_root, t).await,
        None => None,
    };
...
async fn build_worktree_rows(app: &App) -> Vec<crate::app::WorktreeRow> {
    let project_root = app.project_root().to_path_buf();
    let entries = keel_runtime::worktree::list_worktrees(&project_root).await;
...
fn derive_slug_from_entry(e: &keel_runtime::WorktreeListEntry) -> String {
    if let Some(branch) = e.branch.as_deref() {
        return keel_runtime::worktree::slugify(branch);
    }
    if e.detached {
        return keel_runtime::worktree::slugify(
    ...
    match code {
        ...
        KeyCode::Enter => match app.switcher_confirm() {
            crate::app::SwitcherConfirm::OpenCreateForm => {
                let project_root = app.project_root().to_path_buf();
                let branches = keel_runtime::list_branches(&project_root).await;
                ...
                let parent = keel_runtime::git_toplevel(&project_root)
                    .await
