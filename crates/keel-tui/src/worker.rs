...
use keel_container::{Backend, ServiceStatus};
use std::sync::Arc;
...
pub enum WorkerSnapshot {
    ...
    TmuxWindows(Vec<crate::app::TmuxWindow>),
}
...
async fn poll_tmux(
    ...
    let windows = match crate::terminal::list_tmux_windows(session).await {
        crate::terminal::WindowList::Ok(w) => w,
        crate::terminal::WindowList::NoSession(_)
        | crate::terminal::WindowList::SpawnFailed(_) => Vec::new(),
    };
...
mod tests {
    use super::*;
    use keel_container::null::NullBackend;
    ...
    async fn poke_tmux_emits_window_snapshot() {
        ...
        handle
            ...
            .send(WorkerCommand::SetTmuxSession(Some(
                "keel-bell-test-nonexistent".into(),
            )))
            .unwrap();
