...
//! task with a [`ChannelSink`](keel_runtime::ChannelSink) feeding output
...
use keel_runtime::{Executor, OutputLine, OutputStream, RuntimeError};
...
impl RunState {
    ...
    /// straight from [`keel_container::Backend::service_action`] and
    ...
    pub fn spawn(executor: &Executor, name: impl Into<String>, args: Vec<String>) -> Self {
        let (sink, output_rx) = keel_runtime::ChannelSink::new_pair();
