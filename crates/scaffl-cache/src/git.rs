//! Content-addressed git cache for scaffl.
//!
//! scaffl owns the cache — it never delegates to a third-party tool.
//! Every cache lives at `<project>/.scaffl/cache/<kind>/<key>/`, one
...
/// `.scaffl/cache/`. New variants only need to map to a path segment.
...
/// repo/source types into this on the way in so `scaffl-cache` does
