...
use scaffl_agents::{AgentsError, AgentsState, ApplyOptions, FileMode, apply};
use scaffl_config::{
...
fn cfg_with(source_repo: &Path, rev: &str) -> AgentsConfig {
    AgentsConfig {
        ...
        manifest_path: "scaffl-agents.toml".into(),
...
async fn install_then_update_then_orphan_removal() {
    ...
    // skills/{a.md,b.md}.
    write(
        &upstream.path().join("scaffl-agents.toml"),
        r#"
        [[file]]
...
async fn drift_is_detected_and_left_alone_by_default() {
    ...
    // Hand-edit the scaffl-owned file.
