//! `.scaffl/agents.state.json` — what scaffl owns, what it pinned to,
...
pub struct AppliedFile {
    ...
    pub src: String,
    /// SHA-256 of the bytes scaffl last wrote. `None` for `mode =
        ...
        if parsed.version != SCHEMA_VERSION {
            return Err(AgentsError::Io {
                ...
                source: std::io::Error::new(
                    ...
                    format!(
                        "agents.state.json has version {}, expected {}. Remove the file and re-run `scaffl agents install`.",
...
fn state_path(project_root: &Path) -> PathBuf {
    project_root.join(".scaffl").join(STATE_FILE)
}
...
mod tests {
    ...
    fn version_mismatch_is_an_error() {
        let dir = TempDir::new().unwrap();
        std::fs::create_dir_all(dir.path().join(".scaffl")).unwrap();
        std::fs::write(
            state_path(dir.path()),
