...
use crate::manifest::{FileMode, UpstreamManifest};
use scaffl_config::{ResolvedOverride, SourceSpec};
...
/// `scaffl-cache`; `subpath` (if any) is joined to it before any src
/// is resolved.
...
mod tests {
    use super::*;
    use crate::manifest::{DirMapping, FileMapping, UpstreamManifest};
    use scaffl_config::MappingOverride;
    ...
    fn skip_override_drops_mapping() {
        ...
        let source = dummy_source(
            ...
            vec![MappingOverride {
                dest: "AGENTS.md".into(),
                action: Some(scaffl_config::MappingOverrideKind::Skip),
                relocate: None,
            }],
        );
        let entries = expand_source(&source, dir.path(), &manifest).unwrap();
